...
const useWorkspaceStore = create((set, get) => ({
  ...
  setCurrentWorkspace: (workspace) => set({
    ...
    nodes: workspace?.nodes?.map(n => ({ ...n, id: String(n.id) })) || [],
    edges: workspace?.edges?.map((e, index) => ({
      ...e,
      id: e.id ? String(e.id) : `edge-${index}`,
      source: String(e.source || e.from),
      target: String(e.target || e.to),
      sourceHandle: e.sourceHandle || 'fallback-source',
      targetHandle: e.targetHandle || 'fallback-target',
    })) || [],
